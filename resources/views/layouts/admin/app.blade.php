...
<!-- End Footer -->
    ...
    <div class="modal fade" id="popup-modal-laundry">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-body">
                    <div class="row">
                        <div class="col-12">
                            <center>
                                <h2 style="color: rgba(96,96,96,0.68)">
                                    <i class="tio-notifications-on"></i> You have new laundry order, Check Please.
                                </h2>
                                <hr>
                                <button onclick="check_order()" class="btn btn-primary">Ok, let me check</button>
                            </center>
...
<script>
    ...
    @if(\App\CentralLogics\Helpers::module_permission_check('order'))
        ...
        setInterval(function () {
            $.get({
                ...
                success: function (response) {
                    ...
                    console.log(new_order_type);
                    if (data.new_order > 0) {
                        playAudio();
                        if (new_order_type == 'laundry_order') {
                            $('#popup-modal-laundry').appendTo("body").modal('show');
                        }else{
        ...
        function check_order() {
            if(new_order_type == 'laundry_order')
            {
                location.href = "{{route('admin.laundry.order.list',['status'=>'pending'])}}";
            }
            else
            {
    ...
    function route_alert(route, message, title="{{__('messages.are_you_sure')}}", processing = false) {
        if (processing) {
                Swal.fire({
                    ...
                    inputPlaceholder: "{{ trans('Enter processing time') }}",
                    input: 'text',
                    html: message + '<br/>' + '<label>{{ trans('Enter Processing time in minutes') }}</label>',
