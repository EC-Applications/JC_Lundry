...
@section('content')
    <?php 
        ...
        $deal_order=isset($order->details[0]->deal)?true:false;    
    ?>
            ...
            <div class="row align-items-center">
                <div class="col-sm mb-2 mb-sm-0">
                    ...
                    <div class="d-sm-flex align-items-sm-center">
                        ...
                        @endif
                        @if($campaign_order || $deal_order)
                            <span class="badge badge-soft-success ml-sm-3">
                                <span class="legend-indicator bg-success"></span>{{__($campaign_order ? 'messages.campaign_order' : 'messages.deal_order')}}
                            </span>
                        @endif
                        @if($order->edited)
                    ...
                    <div class="mt-2">
                        ...
                        <div class="hs-unfold float-right">
                            ...
                            <a class="btn btn-sm btn-danger" onclick="order_status_change_alert('{{route('vendor.order.status',['id'=>$order['id'],'order_status'=>'canceled'])}}', '{{__('messages.order_canceled_confirmation')}}', false, false, true)" href="javascript:">{{__('messages.cancel_this_order')}}</a>
                            @endif
                            @elseif ($order['order_status']=='confirmed' || $order['order_status']=='accepted')
                            <a class="btn btn-sm btn--primary w-100 mb-3"
                            onclick="order_status_change_alert('{{ route('vendor.order.status', ['id' => $order['id'], 'order_status' => 'processing']) }}','{{ trans('Change status to cooking ?') }}', verification = false, {{ $max_processing_time }})"
                            href="javascript:">{{ trans('messages.Proceed_for_cooking') }}</a>                            @elseif ($order['order_status']=='processing')
            ...
            <div class="col-lg-8 mb-3 mb-lg-0">
                ...
                <div class="card mb-3 mb-lg-5">
                    ...
                    <div class="card-header" style="display: block!important;">
                        ...
                        <div class="row">
                            @if($order->order_status == 'canceled' && $order->order_cancellation_reason)
                            <div class="col-12 pt-2">
                                <h6>
                                    <span style="color: #fd0000;">{{__('messages.order_cancellation_reason')}} :</span> {{$order['order_cancellation_reason']}}
                                </h6>
                                <hr>
                            </div>
                            @endif
                    ...
                    @foreach($order->details as $key=>$detail)
                                    ...
                                    <div class="media-body">
                                        <div class="row">
                                            <div class="col-md-6 mb-3 mb-md-0">
                                                <strong> {{Str::limit($detail->campaign['title'], 25, '...')}}</strong><br>
                        ...
                        @elseif ($detail->deal)
                            ...
                            <div class="media">
                                <div class="avatar avatar-xl mr-3">
                                    <img class="img-fluid"
                                            src="{{asset('storage/app/public/deal')}}/{{$detail->deal->image}}"
                                            onerror="this.src='{{asset('public/assets/admin/img/160x160/img2.jpg')}}'"
                                            alt="{{$detail->deal->title}}">
                                </div>
                                <div class="media-body">
                                    <div class="row">
                                        <div class="col-md-6 mb-3 mb-md-0">
                                            <strong> {{Str::limit($detail->deal->title, 20, '...')}}</strong><br>
                                            @php($deal = json_decode($detail->food_details,true))
                                            
                                            <strong><u>{{__('messages.options')}} : </u></strong>
                                            @foreach($deal['options'] as $index=>$option)
                                                @if ($index != 0)
                                                    <hr>    
                                                @endif
                                                <div class="font-size-sm text-body">
                                                    <span>{{$option['title']}} :  </span>
                                                    <span class="font-weight-bold">{{Str::limit($option['items'][0]['name'],15, '...')}}</span>
                                                </div>
                                                @if(count($option['addons'])>=0)<strong><u>{{__('messages.addons')}} : </u></strong>@endif
                                                @foreach($option['addons'] as $addon)
                                                    <div class="font-size-sm text-body">
                                                        <span>{{Str::limit($addon['name'],20,'...')}} :  </span>
                                                        <span class="font-weight-bold">
                                                            {{$deal['quantity']}} x {{\App\CentralLogics\Helpers::format_currency($addon['price'])}}
                                                        </span>
                                                    </div>
                                                    @php($total_addon_price+=$addon['price']*$deal['quantity'])
                                                @endforeach                                                
                                            @endforeach
                                        </div>
...
@push('script_2')
    <script>
        function order_status_change_alert(route, message, verification, processing = false, canceled = false) {
            if(verification)
            {
                Swal.fire({
                    title: "{{trans('messages.enter_verification_code')}}",
                    input: 'text',
                    inputAttributes: {
                        autocapitalize: 'off'
                    },
                ...
                Swal.fire({
                    ...
                    confirmButtonText: '{{ trans('messages.submit') }}',
                    inputPlaceholder: "{{ trans('messages.enter_processing_time') }}",
                    input: 'text',
                    html: message + '<br/>'+'<label>{{ trans('messages.enter_processing_time_in_minutes') }}</label>',
                ...
                Swal.fire({
                    ...
                    cancelButtonText: '{{ trans('messages.cancel') }}',
                    confirmButtonText: '{{ trans('messages.submit') }}',
                    inputPlaceholder: "{{ trans('messages.enter_cancellation_reason') }}",
                    input: 'text',
                    preConfirm: (order_cancellation_reason) => {
                        location.href = route + '&order_cancellation_reason=' + order_cancellation_reason;
                    },
                    allowOutsideClick: () => !Swal.isLoading()
                })
            } else{
