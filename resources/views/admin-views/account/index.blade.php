            ...
            <div class="card-body">
                <form action="{{ route('admin.account-transaction.store') }}" method='post' id="add_transaction">
                    ...
                    <div class="row">
                        <div class="col-md-4">
                            <div class="form-group">
                                <label class="input-label" for="type">{{ __('messages.type') }}<span
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                <label class="input-label" for="restaurant">{{ __('messages.restaurant') }}<span
                                        class="input-label-secondary"></span></label>
                                <select id="restaurant" name="restaurant_id"
                                    data-placeholder="{{ __('messages.select') }} {{ __('messages.restaurant') }}"
                                    onchange="getAccountData('{{ url('/') }}/admin/vendor/get-account-data/',this.value,'restaurant')"
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                <label class="input-label" for="deliveryman">{{ __('messages.deliveryman') }}<span
                                        class="input-label-secondary"></span></label>
                                <select id="deliveryman" name="deliveryman_id"
                                    data-placeholder="{{ __('messages.select') }} {{ __('messages.deliveryman') }}"
                    ...
                    <div class="row">
                        <div class="col-md-4">
                            <div class="form-group">
                                <label class="input-label" for="method">{{ __('messages.method') }}<span
                                        class="input-label-secondary"></span></label>
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                <label class="input-label" for="ref">{{ __('messages.reference') }}<span
                                        class="input-label-secondary"></span></label>
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                <label class="input-label" for="amount">{{ __('messages.amount') }}<span
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        ...
                        <div class="col-md-6">
                            <form method="GET" action="{{ route('admin.account-transaction.index') }}">
                                    ...
                                    @foreach ($account_transaction as $k => $at)
                                        <tr>
                                            ...
                                            <td>
                                                @if ($at->restaurant)
                                                    <a
                                                        href="{{ route('admin.vendor.view', [$at->restaurant['id']]) }}">{{ Str::limit($at->restaurant->name, 20, '...') }}</a>
                                                @elseif($at->deliveryman)
                                                    <a
                                                        href="{{ route('admin.delivery-man.preview', [$at->deliveryman->id]) }}">{{ $at->deliveryman->f_name }}
        ...
        function getAccountData(route, data_id, type) {
            $.get({
                ...
                success: function(data) {
                    $('#account_info').html('({{ __('messages.cash_in_hand') }}: ' + data.cash_in_hand +
                        ' {{ __('messages.earning_balance') }}: ' + data.earning_balance + ')');
                },
            });
        }
    </script>
