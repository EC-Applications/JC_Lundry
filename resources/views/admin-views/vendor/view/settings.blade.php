                    ...
                    <div class="card h-100">
                        ...
                        <div class="card-body">
                            <div class="row justify-content-md-center">
                                ...
                                <div class="col-xl-4 col-md-4 col-sm-6 col-6">
                                    <div class="form-group">
                                        ...
                                        <span class="pr-2 text-capitalize">{{__('messages.self_delivery_system')}}:</span>
                                            <input type="checkbox" class="toggle-switch-input" onclick="location.href='{{route('admin.vendor.toggle-settings',[$restaurant->id,$restaurant->self_delivery_system?0:1, 'self_delivery_system'])}}'" id="self_delivery_system" {{$restaurant->self_delivery_system?'checked':''}} {{$restaurant->restaurant_wise_sc_status ? 'disabled' : ''}}>
                                            <span class="toggle-switch-label" @if($restaurant->restaurant_wise_sc_status) data-toggle="tooltip" data-placement="top" title="{{trans('messages.self_delivery_restaurant_sc_warning')}}" @endif>
                                                <span class="toggle-switch-indicator"></span>
                                            </span>
                                        </label>
                                    </div>
                                ...
                                <div class="col-xl-4 col-md-4 col-sm-6 col-6">
                                    <div class="form-group">
                                        <label class="toggle-switch toggle-switch-sm d-flex justify-content-between border border-secondary rounded px-4 form-control" for="edit_agreement">
                                        <span class="pr-2 text-capitalize">{{__('messages.can_edit_agreement')}}:</span>
                                            <input type="checkbox" class="toggle-switch-input" onclick="location.href='{{route('admin.vendor.toggle-settings',[$restaurant->id,$restaurant->edit_agreement?0:1, 'edit_agreement'])}}'" id="edit_agreement" {{$restaurant->edit_agreement?'checked':''}}>
                                            <span class="toggle-switch-label">
                                                <span class="toggle-switch-indicator"></span>
                                            </span>
                                        </label>
                            ...
                            <form action="{{route('admin.vendor.update-settings',[$restaurant['id']])}}" method="post"
                                ...
                                <div class="row">
                                    ...
                                    <div class="form-group col-md-6">
                                        ...
                                        <input type="text" name="maximum_delivery_time" class="form-control" placeholder="40" required value="{{explode('-',$restaurant->delivery_time)[1]}}">
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-6">
                                ...
                                <div class="card">
                                    <div class="card-header p-0 pt-2">
                                        <div class="col-12">
                                            <label class="d-flex justify-content-between switch toggle-switch-sm text-dark" for="restaurant_wise_sc_status">
                                                <span>{{__('messages.restaurant_wise_delivery_fee')}} <span class="input-label-secondary" title="{{__('messages.if_sales_commission_disabled_system_default_will_be_applicable')}}"><img src="{{asset('/public/assets/admin/img/info-circle.svg')}}" alt="{{__('messages.if_sales_commission_disabled_system_default_will_be_applicable')}}"></span></span>
                                                <input type="checkbox" class="toggle-switch-input" name="restaurant_wise_sc_status" id="restaurant_wise_sc_status" value="1" {{$restaurant->restaurant_wise_sc_status  ?'checked':''}} {{$restaurant->self_delivery_system ? 'disabled' : '' }} >
                                                <span class="toggle-switch-label" @if($restaurant->self_delivery_system) data-toggle="tooltip" data-placement="top" title="{{trans('messages.restaurant_sc_enable_warning')}}" @endif>
                                                    <span class="toggle-switch-indicator"></span>
                                                </span>
                                            </label>
                                        </div>
                                    ...
                                    <div class="card-body">
                                        <div class="row">
                                            <div class="form-group col-md-6">
                                                <label class="input-label" for="minimum_delivery_time">{{__('messages.minimum_shipping_charge')}} ({{\App\CentralLogics\Helpers::currency_symbol()}})</label>
                                                <input type="text" name="minimum_shipping_charge" id="minimum_shipping_charge" class="form-control" placeholder="30" step=".01" value="{{$restaurant->minimum_shipping_charge}}" {{$restaurant->restaurant_wise_sc_status ? '' : 'readonly'}}>
                                            </div>
                                            <div class="form-group col-md-6">
                                                <label class="input-label" for="maximum_delivery_time">{{__('messages.per_km_shipping_charge')}} ({{\App\CentralLogics\Helpers::currency_symbol()}})</label>
                                                <input type="text" name="per_km_shipping_charge" id="per_km_shipping_charge" class="form-control" placeholder="10" value="{{$restaurant->per_km_shipping_charge}}" {{$restaurant->restaurant_wise_sc_status ? '' : 'readonly'}}>
                                            </div>
                                        </div>
                                    </div>
...
@push('script_2')
    ...
    <script>
        $(document).on('ready', function () {
            ...
            $("#restaurant_wise_sc_status").on('change', function(){
                if($(this).is(':checked')){
                    $('#minimum_shipping_charge').removeAttr('readonly');
                    $('#per_km_shipping_charge').removeAttr('readonly');
                    $('#minimum_shipping_charge').attr('required', true);
                    $('#per_km_shipping_charge').attr('required', true);
                } else {
                    $('#minimum_shipping_charge').attr('readonly', true);
                    $('#per_km_shipping_charge').attr('readonly', true);
                    $('#minimum_shipping_charge').removeAttr('required');
                    $('#per_km_shipping_charge').removeAttr('required');
                }
            })
