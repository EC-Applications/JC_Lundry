<div>
    <div class="card-header">
        <h3>{{ __('messages.order') }} {{ __('messages.transactions') }}</h3>
    </div>
    {{-- <h3 class="qcont px-3 pt-4">{{ __('messages.order')}} {{ __('messages.transactions')}} ({{trans('messages.cash_in_hand')}})</h3> --}}
    <div class="row justify-content-around mt-2">
        <div class="col-md-3">
            <form method="GET"
                action="{{ route('admin.delivery-man.preview', ['id' => $dm->id, 'tab' => 'transaction', 'sub_tab' => 'order-cash-in-hand']) }}">
        ...
        <div class="col-auto justify-content-right">
            <form method="GET"
                ...
                <div class="input-group mb-3">
                    ...
                    <input type="date" name="from" value="{{ request('from') }}" class="form-control" placeholder=""
                        aria-label="" aria-describedby="basic-addon1">
                    <div class="input-group-prepend">
                        <button class="btn btn-outline-secondary" disabled type="button">---</button>
                    </div>
                    <input type="date" name="to" value="{{ request('to') }}" class="form-control" placeholder=""
                        aria-label="" aria-describedby="basic-addon1">
                    <div class="input-group-prepend">
    ...
    <div class="table-responsive">
        <table id="datatable" class="table table-thead-bordered table-align-middle card-table" style="width: 100%">
            ...
            <tbody>
                ...
                @php(
                $transaction = \App\Models\OrderTransaction::where('delivery_man_id', $dm->id)->whereHas('order', function ($query) {
