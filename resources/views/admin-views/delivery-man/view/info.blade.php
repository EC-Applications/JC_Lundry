            ...
            <div class="card-header">
                <h4 class="page-header-title">
                    ...
                    (@if ($dm->zone)
                        {{ $dm->zone->name }}
                    @else
                        {{ __('messages.zone') . ' ' . __('messages.deleted') }}
                    @endif )
                    @if ($dm->application_status == 'approved')
                        @if ($dm['status'])
                            @if ($dm['active'])
                    ...
                    @else
                        <label
                            class="badge badge-soft-{{ $dm->application_status == 'pending' ? 'info' : 'danger' }}">{{ __('messages.' . $dm->application_status) }}</label>
                    @endif
                </h4>
                ...
                <div class="hs-unfold float-right">
                    <div class="dropdown">
                        <button class="btn btn-secondary dropdown-toggle" type="button" id="dropdownMenuButton"
                            ...
                            {{ __('messages.type') }}
                            ({{ $dm->earning ? __('messages.freelancer') : __('messages.salary_based') }})
                        </button>
                        ...
                        <div class="d-flex align-items-center">
                            ...
                            <div class="d-block">
                                <h4 class="display-2 text-dark mb-0">
                                    {{ count($dm->rating) > 0 ? number_format($dm->rating[0]->average, 2, '.', ' ') : 0 }}</h4>
                    ...
                    <div class="card-body">
                        <div class="row no-gutters align-items-center">
                            <div class="col mr-2">
                                ...
                                <div class="for-card-count">{{ $dm->orders->count() }}
                                </div>
                            </div>
                        </div>
                    </div>
...
@push('script_2')
    <script>
        ...
        // function suspentionStore(form_id = 'suspensionCreate') {
        //     $.ajaxSetup({
        //         headers: {
        ...
        //     });
        //     $.post({
        //         url: '{{ route('admin.delivery-man.suspension-create') }}',
        ...
        //         },
        //         complete: function() {
        //             toastr.success('{{__('messages.suspenson_stored')}}', {
        //                 CloseButton: true,
        //                 ProgressBar: true
        //             });
        //         }
