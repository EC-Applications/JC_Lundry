        ...
        <div class="card mb-3 mb-lg-5 mt-2">
            <div class="card-header">
                <div class="search--btn-wrapper">
                    ...
                    <form method="GET" action="{{route('admin.delivery-man.preview', ['id'=>$dm->id, 'tab'=> 'timelog'])}}">
                    ...
                    <table id="datatable"
                        ...
                        <thead class="thead-light">
                            <tr>
                                ...
                                <th>{{__('messages.total_working_hour')}}</th>
                                <th>{{__('messages.shift_type')}}</th>
                        ...
                        @foreach($timelogs as $k=>$log)
                            <tr>
                                ...
                                <td>{{$dm->zone->name}}</td>
                                <td>
                                    <span class="bg-gradient-light text-dark">{{date(config('timeformat'), strtotime($log->start_time))}}</span>
                                </td>
                                <td>
                                    <span class="bg-gradient-light text-dark">{{date(config('timeformat'), strtotime($log->end_time))}}</span>
                                </td>
                                <td>{{$log->duration}}</td>
                                @php($hrs = (intdiv($log->total_working_time, 60))  == 0?'':intdiv($log->total_working_time, 60).' Hrs ')
                                @php($total_working_time =$hrs. ' ' .($log->total_working_time % 60).' mins')
                                <td>{{$total_working_time}}</td>
                                <td>{{isset($log->shift) && $log->shift->is_special == 1 ? __('messages.Special') : __('messages.Normal')}}</td>
                                <td>{{trans("messages.{$log->status}")}}</td>
                            </tr>
                        @endforeach
                        </tbody>
                    </table>
