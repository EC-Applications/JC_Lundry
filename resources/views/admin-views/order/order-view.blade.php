            ...
            <div class="row align-items-center">
                <div class="col-sm mb-2 mb-sm-0">
                    ...
                    <div class="mt-2">
                        ...
                        @if($order->order_status != 'refunded' && $order->order_status != 'failed' && $order->order_status != 'delivered')
                            ...
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu text-capitalize" aria-labelledby="dropdownMenuButton">
                                    ...
                                    <a class="dropdown-item {{$order['order_status']=='confirmed'?'active':''}}"
                                       ...
                                       onclick="route_alert('{{ route('admin.order.status', ['id' => $order['id'], 'order_status' => 'processing']) }}', '{{ trans('Change status to processing ?') }}','{{ trans('Are you sure?') }}', '{{ $max_processing_time }}')"
                                       href="javascript:">
                                       {{ trans('messages.processing') }}</a>
                    ...
                    <div class="card-header" style="display: block!important;">
                        <div class="row">
                            <div class="col-12 pb-2 border-bottom  d-flex justify-content-between">
                                ...
                                @if (!$editing && in_array($order->order_status, ['pending','confirmed','processing','accepted']) && !$campaign_order && !$deal_order)
                        ...
                        <!-- food cart -->
                        @if($editing && !$campaign_order && !$deal_order)
                        ...
                        @elseif ($detail->deal)
                            ...
                            <div class="media">
                                ...
                                <div class="media-body">
                                    <div class="row">
                                        <div class="col-md-6 mb-3 mb-md-0">
                                            <strong> {{$detail->deal->title}}</strong><br>
                                            ...
                                            @foreach($deal['options'] as $index=>$option)
                                                ...
                                                <div class="font-size-sm text-body">
                                                    ...
                                                    <span class="font-weight-bold">{{$option['items'][0]['name']}}</span>
                                                </div>
                ...
                <div class="card mb-2">
                    ...
                    <div class="card-body">
                        <dl class="row">
                            ...
                            <dd class="col-sm-4">{{date(config('timeformat'), strtotime($order->created_at))}}</dd>
                            <dt class="col-sm-8">{{__('messages.order_accepted_by_rider')}}:</dt>
                            <dd class="col-sm-4">{{$order->accepted ? date(config('timeformat'), strtotime($order->accepted)) : trans('messages.na')}}</dd>
                            <dt class="col-sm-8">{{__('messages.order_accepted_by_vendor')}}:</dt>
                            <dd class="col-sm-4">{{$order->processing ? date(config('timeformat'), strtotime($order->processing)) : trans('messages.na')}}</dd>
                            {{-- <dt class="col-sm-8">{{__('messages.rider_reached_pickup_location')}}:</dt>
                            <dd class="col-sm-4">{{$order->accepted ? date(config('timeformat'), strtotime($order->accepted)) : trans('messages.na')}}</dd> --}}
                            <dt class="col-sm-8">{{__('messages.rider_picks_up_order')}}:</dt>
                            <dd class="col-sm-4">{{$order->picked_up ? date(config('timeformat'), strtotime($order->picked_up)) : trans('messages.na')}}</dd>
                            ...
                            <dd class="col-sm-4">{{$order->delivered ? date(config('timeformat'), strtotime($order->delivered)) : trans('messages.na')}}</dd>
                        </dl>
                    </div>
                    <!-- End Body -->
                </div>
