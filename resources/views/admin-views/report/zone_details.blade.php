...
@section('title', 'Zone Details List')
        ...
        <div class="page-header">
            <div class="row align-items-center">
                <div class="col-sm mb-2 mb-sm-0">
                    <h1 class="page-header-title"><i class="tio-filter-list"></i> {{ __('messages.zone_details') }}</h1>
                </div>
            </div>
        </div>
            ...
            <div class="col-sm-12 col-lg-12 mb-3 mb-lg-2">
                ...
                <div class="card">
                    <div class="card-header">
                        <h5>{{ __('messages.zone_details') }}<span class="badge badge-soft-dark ml-2"
                                id="itemCount">{{ $zone_reports->total() }}</span></h5>
                        {{-- <form id="search-form">
                            @csrf
                            <!-- Search -->
                    ...
                    <div class="table-responsive datatable-custom">
                        <table id="columnSearchDatatable"
                            ...
                            <thead class="thead-light">
                                <tr>
                                    <th>{{ __('messages.#') }}</th>
                                    <th>{{ __('messages.zone') }} {{ __('messages.name') }}</th>
                                    <th>{{ __('messages.zone') }} {{ __('messages.deliverymans') }}</th>                                    
                                    <th>{{ __('messages.zone') }} {{ __('messages.fill_rates') }}</th>
                                    <th>{{ __('messages.zone') }} {{ __('messages.orders') }}</th>
                                    <th>{{ __('messages.assigned_orders') }}</th>
                                    <th>{{ __('messages.unassigned_orders') }}</th>
                                    <th>{{ __('messages.delivery_time') }} {{__('messages.dt_in_min')}}</th>
                            ...
                            <tbody id="set-rows">
                                @foreach ($zone_reports as $key => $report)
                                    <tr>
                                        <td>{{ $loop->index + 1 }}</td>
                                        <td>
                                            <span class="d-block text-body">
                                                {{ Str::limit($report->name, 25, '...') }}
                                            </span>
                                        </td>
                                        <td>
                                            <span
                                                class="bg-gradient-light text-dark">{{ $report->zone_riders }}</span>
                                        </td>
                                        <td>
                                            <span class="bg-gradient-light text-dark">
                                                {{ ($report->delivery_type_order_count / 100) * $report->assigned_orders }}%</span>
                                        </td>
                                        <td>
                                            <span class="bg-gradient-light text-dark">
                                                {{ $report->total_orders }}</span>
                                        </td>
                                        <td>
                                            <span
                                                class="bg-gradient-light text-dark">{{ $report->assigned_orders }}</span>
                                        </td>
                                        <td>
                                            <span
                                                class="bg-gradient-light text-dark">{{ $report->unassigned_orders }}</span>
                                        </td>
                                        <td>
                                            <span
                                                class="bg-gradient-light text-dark">{{ number_format($report->avg_delivery_time, 2, '.', ',') }}</span>
                                        </td>
                                        {{-- <td>
