...
@section('title',trans('messages.generate_barcode_for_items'))
    ...
    <div class="content container-fluid">
        ...
        <div class="page-header">
            <div class="row align-items-center">
                <div class="col-sm mb-2 mb-sm-0">
                    <h1 class="page-header-title"><i class="tio-edit"></i> {{__('messages.generate_barcode_for_items')}}</h1>
                </div>
            </div>
        </div>
        <!-- End Page Header -->
        <div class="row gx-2 gx-lg-3">
            <div class="col-sm-12 col-lg-12 mb-3 mb-lg-2">
                <div class="card">
                    <div class="card-body">
                        <form action="javascript:" id="barcode-form" method="GET">
                            <div class="row">
                                <div class="col-md-4 offset-md-4">
                                    <div class="form-group">
                                        <label class="input-label" for="exampleFormControlInput1">{{__('messages.enter_package_barcode')}}</label>
                                        <input type="text" value="{{old('bar_code',$order->bar_code??null)  }}" name="bar_code" class="form-control" placeholder="{{__('messages.barcode')}}" required>
                                    </div>
                                    {{-- <button type="submit" class="btn btn-block btn-primary">{{__('messages.submit')}}</button> --}}
                                </div>
                            </div>
                        </form>
                    </div>
...
<script>
    ...
    document.getElementById('barcode-form').addEventListener('input', function (event) {
        ...
        if (barcode.length >= 6) {
            ...
            $.ajax({
                    url: '{{ route('admin.laundry.in-house-tracking.get-items-barcodes') }}',
                    method: 'GET',
                    data: {
                        bar_code: barcode
                    },
