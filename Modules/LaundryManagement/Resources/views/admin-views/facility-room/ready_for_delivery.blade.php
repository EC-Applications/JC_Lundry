...
@section('title',trans('messages.ready_for_delivery'))
        ...
        <div class="page-header">
            <div class="row align-items-center">
                <div class="col-sm mb-2 mb-sm-0">
                    <h1 class="page-header-title"><i class="tio-edit"></i> {{__('messages.update_laundry_product_status')}}</h1>
                </div>
            </div>
        </div>
        <!-- End Page Header -->
        <div class="row gx-2 gx-lg-3">
            <div class="col-sm-12 col-lg-12 mb-3 mb-lg-2">
                <div class="card mx-auto">
                    <div class="card-body">
                        <form action="javascript:" id="barcode-form" method="POST">
                            @csrf

                            <div class="row">
                                <div class="col-md-4 offset-md-4">
                                    <div class="form-group">
                                        <label class="input-label" for="exampleFormControlInput1">{{__('messages.enter_product_barcode')}}</label>
                                        <input type="text" value="{{old('bar_code',$product->bar_code??null)  }}" name="bar_code" class="form-control" placeholder="{{__('messages.barcode')}}" required>
                                    </div>
...
<script>
    ...
    document.getElementById('barcode-form').addEventListener('input', function (event) {
        ...
        if (barcode.length >= 6) {
            ...
            $.ajax({
                    url: '{{ route('admin.laundry.in-house-tracking.ready-for-delivery') }}',
                    method: 'POST',
                    data: {
                        bar_code: barcode
                    },
