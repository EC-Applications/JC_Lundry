    ...
    <div class="content container-fluid">
        ...
        <div class="page-header">
            <div class="row align-items-center">
                <div class="col-sm mb-2 mb-sm-0">
                    <h1 class="page-header-title"><i class="tio-add-circle-outlined"></i> {{__('messages.add_new_laundry')}} {{__('messages.banner')}}</h1>
                </div>
            </div>
        </div>
        <!-- End Page Header -->
        <div class="row gx-2 gx-lg-3">
            <div class="col-sm-12 col-lg-12 mb-3 mb-lg-2">
                <div class="card">
                    <div class="card-body">
                        <form action="{{route('admin.laundry.banner.store')}}" method="post" enctype="multipart/form-data">
                            @csrf
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="services_id" id="zone" class="form-control js-select2-custom">
                                            <option disabled selected>---{{__('messages.select_service')}}---</option>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <span class="media align-items-center">
                                            <img class="avatar avatar-lg mr-3 avatar--3-1" src="{{asset('storage/app/public/laundry-banners')}}/{{$banner['image']}}"
                                    ...
                                    <td>{{$banner->service->name}}</td>
                                    <td>
                                        <label class="toggle-switch toggle-switch-sm" for="statusCheckbox{{$banner->id}}">
                                            <input type="checkbox" onclick="location.href='{{route('admin.laundry.banner.status',[$banner['id'],$banner->status?0:1])}}'" class="toggle-switch-input" id="statusCheckbox{{$banner->id}}" {{$banner->status?'checked':''}}>
                                    ...
                                    <td>
                                        <a class="btn btn-sm btn-white" href="{{route('admin.laundry.banner.edit',[$banner['id']])}}"title="{{__('messages.edit')}} {{__('messages.banner')}}"><i class="tio-edit"></i>
                                        </a>
                                        ...
                                        <form action="{{route('admin.laundry.banner.delete',[$banner['id']])}}"
...
<script>
    ...
    <script>
        $('#search-form').on('submit', function (e) {
            ...
            $.post({
                url: '{{route('admin.laundry.banner.search')}}',
                data: formData,
                cache: false,
                contentType: false,
                processData: false,
