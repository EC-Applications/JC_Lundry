        ...
        <div class="page-header">
            <div class="row align-items-center">
                <div class="col-sm mb-2 mb-sm-0">
                    <h1 class="page-header-title"><i class="tio-filter-list"></i>{{__('messages.laundry')}} {{__('messages.deliverymen')}}</h1>
                </div>
                ...
                <div class="col-sm-auto" style="width: 306px;">
                    <select name="zone_id" class="form-control js-select2-custom"
                            onchange="set_zone_filter('{{route('admin.laundry.delivery-man.list')}}', this.value)">
            ...
            <div class="col-sm-12 col-lg-12 mb-3 mb-lg-2">
                ...
                <div class="card">
                    ...
                    <div class="card-header p-1">
                        <h5>{{__('messages.laundry')}} {{__('messages.deliveryman')}} {{__('messages.list')}}<span class="badge badge-soft-dark ml-2" id="itemCount">{{$delivery_men->total()}}</span></h5>
                    ...
                    <div class="table-responsive datatable-custom">
                        <table id="columnSearchDatatable"
                            ...
                            <tr>
                                ...
                                <th class="text-capitalize">{{__('messages.code')}}</th>
                                <th class="text-capitalize">{{__('messages.availability')}} {{__('messages.status')}}</th>
                                <th class="text-capitalize">{{__('messages.currently_assign_orders')}}</th>
                            ...
                            @foreach($delivery_men as $key=>$dm)
                                <tr>
                                    ...
                                    <td>
                                        <a class="media align-items-center" href="{{route('admin.laundry.delivery-man.preview',[$dm['id']])}}">
                                    ...
                                    <td>
                                        @if($dm->laundry_zone)
                                        <label class="badge badge-soft-info">{{$dm->laundry_zone->name}}</label>
                                        @else
                                        <label class="badge badge-soft-warning">{{__('messages.zone').' '.__('messages.deleted')}}</label>blank
                                        @endif
                                    ...
                                    <td class="text-center">
                                        @if($dm->application_status == 'approved')
                                            @if($dm->active && !$dm->suspension_log)
                                    ...
                                    <td class="text-center" title="{{__('messages.currently_assign_orders')}}">
                                        {{$dm->current_orders}}
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <a class="btn btn-sm btn-white" href="{{route('admin.laundry.delivery-man.edit',[$dm['id']])}}" title="{{__('messages.edit')}}"><i class="tio-edit"></i>
                                        </a>
                                        ...
                                        <form action="{{route('admin.laundry.delivery-man.delete',[$dm['id']])}}" method="post" id="delivery-man-{{$dm['id']}}">
                                            @csrf @method('delete')
                                        </form>
                                    </td>
                                </tr>
...
@push('script_2')
    ...
    <script>
        $('#search-form').on('submit', function (e) {
            ...
            $.post({
                url: '{{route('admin.laundry.delivery-man.search')}}',
                data: formData,
                cache: false,
                contentType: false,
                processData: false,
