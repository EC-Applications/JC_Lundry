...
{
    public static function get_restaurants($zone_id, $filter, $type, $limit = 10, $offset = 1)
    {
        // ST_Distance_Sphere(
    ...
    public static function get_latest_restaurants($zone_id, $type, $discount=false, $limit = 10, $offset = 1)
    {
        $paginator = Restaurant::selectRaw('*, IF((opening_time < "'.now()->format('H:i').'" and closeing_time > "'.now()->format('H:i').'"), true, false) as open')
        ->with(['discount'=>function($q){
            return $q->validate();
    ...
    public static function get_popular_restaurants($zone_id, $type, $limit = 10, $offset = 1)
    {
        $paginator = Restaurant::selectRaw('*, IF((opening_time < "'.now()->format('H:i').'" and closeing_time > "'.now()->format('H:i').'"), true, false) as open')
        ->with(['discount'=>function($q){
            return $q->validate();
    ...
    public static function get_discounted_restaurants($zone_id, $type, $limit, $offset)
    {

        $paginator = Restaurant::selectRaw('`restaurants`.*, `discounts`.`discount` as restaurant_discount, IF((`restaurants`.`opening_time` < "'.now()->format('H:i').'" and `restaurants`.`closeing_time` > "'.now()->format('H:i').'"), true, false) as open')
        ->join('discounts', 'restaurants.id', '=', 'discounts.restaurant_id')
        ->where('zone_id', $zone_id)
        ->Active()
        ->type($type)
    ...
    public static function search_restaurants($name, $zone_id, $type, $category_id= null,$limit = 10, $offset = 1)
    {
        $key = explode(' ', $name);
