...
class ProductLogic
{
    ...
    public static function get_latest_products($limit, $offset, $restaurant_id, $category_id, $type)
    {
        $paginator = Food::active()->type($type)
            ->join('categories as cat', 'food.category_id', '=', 'cat.id')
            ->selectRaw('`food`.`id`, `food`.`name`, `food`.`description`, `food`.`image`, `category_id`, `category_ids`, `variations`, `add_ons`, `attributes`, `choice_options`, `price`, `tax`, `tax_type`, `discount`, `discount_type`, `available_time_starts`, `available_time_ends`, `veg`, `food`.`status`, `restaurant_id`, `food`.`created_at`, `food`.`updated_at`, `order_count`, `avg_rating`, `rating_count`, `rating`, `food`.`priority`, IF((`cat`.`parent_id`="0"), `cat`.`priority`,(select `categories`.`priority` from `categories` where `categories`.`id` = `cat`.`parent_id`)) as category_priority')
            ->when(config('module.current_module_id'), function($query){
                $query->module(config('module.current_module_id'));
            });
        if ($category_id != 0) {
            $paginator = $paginator->whereHas('category', function ($q) use ($category_id) {
                return $q->whereId($category_id);
            });
        }
        $paginator = $paginator->where('restaurant_id', $restaurant_id)
            ->latest('category_priority')
            ->latest('food.priority')
    ...
    public static function popular_products($zone_id, $type, $limit = null, $offset = null)
    {
        ...
        ->type($type)->whereHas('restaurant', function ($q) use ($zone_id) {
            $q->where('zone_id', $zone_id)->Weekday();
    ...
    public static function most_reviewed_products($zone_id, $type, $limit = null, $offset = null)
    {
        if ($limit != null && $offset != null) {
            $paginator = Food::whereHas('restaurant', function ($q) use ($zone_id) {
                $q->where('zone_id', $zone_id)->Weekday();
