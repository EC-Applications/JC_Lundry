...
class CustomerController extends Controller
{
    ...
    public function add_new_address(Request $request)
    {
        ...
        $store_type = $request->header('storeType');

        if(!in_array($store_type, ['food', 'laundry']))
        {
            $errors = [];
            array_push($errors, ['code' => 'module_error', 'message' => trans('messages.module_not_found')]);
            return response()->json([
                'errors' => $errors
            ], 403);
        }

        $zone = ($store_type == 'food') ? Zone::contains('coordinates', $point)->first() : LaundryZone::contains('coordinates', $point)->first();
        ...
        $address = [
            ...
            'zone_id' => $store_type == 'laundry'?0:$zone->id,
            'laundry_zone_id' => $store_type == 'laundry'?$zone->id:null,
            'created_at' => now(),
            'updated_at' => now()
        ];
