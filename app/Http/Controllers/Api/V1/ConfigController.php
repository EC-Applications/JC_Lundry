...
class ConfigController extends Controller
{
    ...
    public function configuration()
    {
        ...
        return response()->json([
            ...
            'base_urls' => [
                ...
                'deal_image_url' => asset('storage/app/public/deal'),
                'delivery_man_document_url' => asset('storage/app/public/delivery-man'),
                'laundry_banner_image_url' => asset('storage/app/public/laundry-banners'),
                'laundry_items_image_url' => asset('storage/app/public/laundry-items'),
                'services_image_url' => asset('storage/app/public/services'),
            ],
            'country' => BusinessSetting::where(['key' => 'country'])->first()->value,
            'laundry_tax' => (double) BusinessSetting::where(['key' => 'laundry_tax'])->first()->value,
            ...
            'number_of_orders_in_checkout' => (int)BusinessSetting::where(['key' => 'number_of_orders_in_checkout'])->first()->value,
            'number_of_items_per_order' => (int)BusinessSetting::where(['key' => 'number_of_items_per_order'])->first()->value,
            'multi_restaurant_order' => (boolean)BusinessSetting::where('key','multi_restaurant_order')->first()->value,
            'minimum_pickup_time_for_laundry_order' => (int)BusinessSetting::where('key','minimum_pickup_time_for_laundry_order')->first()->value,
        ]);
    }
    ...
    public function get_zone(Request $request)
    {
        ...
        $food_zones = Zone::active()->contains('coordinates', $point)->latest()->get();
        $laundry_zones = LaundryZone::active()->contains('coordinates', $point)->latest()->get();
        $laundry_module = Module::active()->laundry()->first();
        $food_module = Module::active()->notLaundry()->first();

        if(count($food_zones)<1 && count($laundry_zones)<1)
        {
            return response()->json([
                'errors'=>[
        ...
        $active_modules = [];
        if (count($laundry_zones)>0 && !is_null($laundry_module)) {
            array_push($active_modules, $laundry_module);
        }
        if (count($food_zones)>0 && !is_null($food_module)) {
            array_push($active_modules, $food_module);
        }

        $response = [
            'zone_id' => json_encode($food_zones->pluck('id')->toArray()),
            'laundry_zone_id' => json_encode($laundry_zones->pluck('id')->toArray()),
            'zone_data' => ["modules" => $active_modules]
        ];
