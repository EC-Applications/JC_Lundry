...
class DeliverymanController extends Controller
{
    ...
    {
        $dm = DeliveryMan::with(['rating','documents', 'zone'])->where(['auth_token' => $request['token']])->first();
    ...
    public function update_profile(Request $request)
    {
        ...
        try{
            ...
            if($request->document){
                ...
                {
                    $files = [];
                    if(!$data = $dm->documents->where('type',$doc['type'])->first()){
                        $data = new Documents();
                        $data->delivery_man_id=$dm->id;
                        $data->created_at=now();
                    }else{
                        $files = $data->files;
                    }
                      
                    if ($request->hasFile("document.{$k}.files")) {
                        ...
                        $files = [];
                        foreach ($doc['files'] as $key=>$img) {
                            $files[] = Helpers::upload('delivery-man/', $doc['document_type']=='image'?'png':$doc['document_type'], $img, "dm_{$dm->id}_{$doc['type']}_{$doc['data']}_{$key}");
                        }
                    }
                    $data->data = $doc['data'];
                    $data->files = $files;
    ...
    public function activeStatus(Request $request)
    {
        ...
        $dm_logs = $dm->time_logs()->latest('id')->first();

        // dd($dm_logs);
        Helpers::shift_time_log($dm_logs, ($dm->active ? now() : null), ($dm->active ? null : now()));

        return response()->json(['message' => trans('messages.active_status_updated'), 'status' => $dm->active], 200);
    }
    ...
    public function accept_order(Request $request)
    {
        ...
        if($order->payment_method == "cash_on_delivery" && (($cash_in_hand+$order->order_amount) >= $value)){
            return response()->json(['errors' => Helpers::error_formater('dm_max_cash_in_hand',trans('delivery man max cash in hand exceeds'))], 203);
        }
        ...
        OrderLogic::order_log_create_or_update($dm->id,$order->id,1);

        $fcm_token=$order->customer->cm_firebase_token;
        Helpers::dm_time_log($dm->id, date('Y-m-d'), $order->id , now(), null);
    ...
    public function reject_order(Request $request)
    {
        ...
        OrderLogic::order_log_create_or_update($dm->id,$request->order_id,0);
    ...
    public function update_order_status(Request $request)
    {
        ...
        else if($request->status == 'canceled')
        {
            if($order->delivery_man)
            {
                ...
                $dm->current_orders = $dm->current_orders>1?$dm->current_orders-1:0;
                $dm->save();
                Helpers::dm_time_log($dm->id, date('Y-m-d'), $order->id , null , now());
            }
        }
        ...
        $o_status = array('canceled', 'failed', 'delivered');
        
        if(in_array($order->order_status, $o_status)){

            Helpers::dm_time_log($dm->id, date('Y-m-d'), $order->id , null , now());

            Helpers::delivery_time_log(date('Y-m-d H:i:s') , $order->processing_time , $order->processing, date('Y-m-d H:i:s'), $order->id, 'delivered');

        }
        if($order->order_status == 'picked_up'){
            
            Helpers::delivery_time_log(date('Y-m-d H:i:s') , '5' , $order->handover, $order->picked_up, $order->id, 'picked_up');
        }
    ...
    {
        return response()->json(Helpers::get_business_settings('documents_type')??[]);
    }
