...
class SocialAuthController extends Controller
{
    public function social_register(Request $request)
    {
        ...
        try {
            ...
            return response()->json(['errors' => [['code'=>'wc','message'=>'wrong credential.']],'message'=>$e->getMessage()],403);
        }
        if (strcmp($email, $data['email']) === 0) {
            ...
            if (isset($user) == false) {
                if(!isset($data['id']) && !isset($data['kid'])){
                    return response()->json(['errors' => [['code'=>'wc','message'=>'wrong credential.']]],403);
                }
    ...
    public function social_login(Request $request)
    {
        ...
        if (strcmp($email, $data['email']) != 0 || (!isset($data['id']) && !isset($data['kid']))) {
            return response()->json(['error' => trans('messages.email_does_not_match')],403);
        }
