...
class VendorController extends Controller
{
    ...
    public function get_restaurants(Request $request)
    {
        ...
        $data = Restaurant::join('zones', 'zones.id', '=', 'restaurants.zone_id')
            ->when(($zone_ids && !$request->no_zone), function ($query) use ($zone_ids) {
    ...
    public function restaurant_status(Restaurant $restaurant, Request $request)
    {
        ...
        if ($request->self_delivery_system && $request->status && $restaurant->restaurant_wise_sc_status) {
            Toastr::warning(trans('messages.self_delivery_restaurant_sc_warning'));
            return back();
        }
    ...
    public function updateRestaurantSettings(Restaurant $restaurant, Request $request)
    {
        ...
        $restaurant->tax = $request->tax;
        $restaurant->delivery_time = $request->minimum_delivery_time . '-' . $request->maximum_delivery_time;
         $restaurant->restaurant_wise_sc_status = $request->restaurant_wise_sc_status ?? 0;
    ...
    {
        $cat = AddOn::withoutGlobalScope(RestaurantScope::class)
            ->where(['restaurant_id' => $request->restaurant_id])->active()->selectRaw('id, name, CONCAT(name, " (",price," ' . Helpers::currency_code() . ')") as text')->get();
        $res = '';
        foreach ($cat as $row) {
    ...
    public function bulk_import_data(Request $request)
    {
        ...
        foreach ($collections as $key => $collection) {
            if ($collection['ownerFirstName'] === "" || $collection['restaurantName'] === "" || $collection['phone'] === "" || $collection['email'] === "" || $collection['latitude'] === "" || $collection['longitude'] === "" || empty($collection['openingTime']) === "" || empty($collection['closeingTime']) || $collection['zone_id'] === "" || $collection['module_id'] === "") {
