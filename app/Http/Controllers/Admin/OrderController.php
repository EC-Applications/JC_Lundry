...
class OrderController extends Controller
{
    ...
    public function details(Request $request, $id)
    {
        ...
        if (isset($order)) {
            ...
            {
                $deliveryMen = DeliveryMan::doesntHave('suspension_log')->where('zone_id',$order->restaurant->zone_id)->available()->active()->get();
            }
    ...
    public function status(Request $request)
    {
        ...
        if($order->order_status == 'delivered' && $order->delivery_man){

            Helpers::dm_time_log($order->delivery_man->id, date('Y-m-d'), $order->id , null , now());

            Helpers::delivery_time_log(date('Y-m-d H:i:s') , $order->processing_time , $order->processing, date('Y-m-d H:i:s'), $order->id, 'delivered');

        }
        if($order->order_status == 'picked_up'  && $order->delivery_man){
    ...
    public function add_delivery_man($order_id, $delivery_man_id)
    {
        ...
        if($deliveryman)
        {
            ...
            Helpers::dm_time_log($deliveryman->id, date('Y-m-d'), $order->id , now(), null);
